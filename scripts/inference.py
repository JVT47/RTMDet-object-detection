...
def batch_image_files(
    image_files: Iterable[str], batch_size: int
...
def main() -> None:
    ...
    model = make_model(
        model_name="RTMDetL", num_classes=80, model_weights=model_weights, eval=True
    )
    ...
    with torch.inference_mode():
        for image_file_batch in batch_image_files(image_files, batch_size):
            images = list(
                map(
                    lambda file_path: torchvision.io.read_image(file_path).float(),
                    image_file_batch,
                )
            )
            ...
            image_file_names = [
                file.rsplit("/", maxsplit=1)[-1] for file in image_file_batch
            ]
            for file_name, image, det_result in zip(
                image_file_names, images, det_results
            ):
                image = image / 255
