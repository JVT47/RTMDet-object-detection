"""
A script that reads images from images directory, draws model detections to them, and saves
the results to images/results directory. Uses the python implementation for inference with
the given pytorch model weights.
...
def preprocess_images_to_batch(
    images: Iterable[torch.Tensor], preprocessor: RTMDetPreprocessor
) -> torch.Tensor:
    """
...
def draw_bboxes_to_original_image(
    image: torch.Tensor, det_result: DetectionResult, input_size: torch.Size
) -> torch.Tensor:
    """
    Draws the detected bboxes to the original image. Transforms the bbox coordinates relative to the original
    dimensions if needed.
    """
    bboxes = RTMDetPostprocessor.bbox_to_original_image(
        det_result.bboxes, input_size, image.shape
    )
    labels = [
        f"{label.item()}, {score.item():.2f}"
        for label, score in zip(det_result.classes, det_result.scores)
    ]
...
def get_arguments() -> Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the model pytorch weights. If not present model_weights/RTMDetM-coco.pth will be used as default.",
    )
    ...
    if not Path(args.model_weights).exists():
        raise ValueError(f"Did not find the given model weights: {args.model_weights}")
...
def main() -> None:
    ...
    with torch.inference_mode():
        for image_file_batch in batch_image_files(
            image_file_generator, args.batch_size
        ):
            images = list(
                map(
                    lambda file_path: torchvision.io.read_image(str(file_path)).float(),
                    image_file_batch,
                )
            )
            ...
            for filename, image, det_result in zip(
                image_filenames, images, det_results
            ):
                image = image / 255
                image = draw_bboxes_to_original_image(
                    image, det_result, input_size=preprocessor.dest_size
                )
