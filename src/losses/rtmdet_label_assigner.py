...
class RTMDetLabelAssigner:
    ...
    def assign_targets(
        self, ground_truths: list[BBoxLabelContainer], model_output: RTMDetOutput
    ) -> BBoxLabelContainer:
        """
        ...
        for element_gts, pred_bboxes, pred_labels in zip(
            ground_truths, preds.bboxes, preds.labels
        ):
            element_preds = BBoxLabelContainer(pred_bboxes, pred_labels)
            element_targets = self.assign_targets_for_batch_element(
                element_gts, element_preds, grid_points
            )
    ...
    def assign_targets_for_batch_element(
        ...
        element_gts = BBoxLabelContainer(
            element_gts.bboxes.float(), element_gts.labels.float()
        )
    ...
    def simOTA(
        ...
        IoU_scores = torchvision.ops.box_iou(
            ground_truths.bboxes, predictions.bboxes
        )  # Shape (m, n)

        for i, (gt_bbox, gt_label, IoU_score) in enumerate(
            zip(ground_truths.bboxes, ground_truths.labels, IoU_scores)
        ):
            sorted_IoUs, _ = IoU_score.sort(descending=True)
            top_k = max(
                1, int(sorted_IoUs[: self.q].sum().round())
            )  # Note that q <= n. In all practical applications this should hold.

            cost: torch.Tensor = self.cls_cost(
                predictions.labels, gt_label, IoU=IoU_score
            )
    ...
    def _make_assigner_matrix_feasible(
        self, assigner_matrix: torch.Tensor, cost_matrix: torch.Tensor
    ) -> None:
        """
