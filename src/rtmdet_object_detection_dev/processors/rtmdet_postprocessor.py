...
class RTMDetPostprocessor:
    """Postprocessor that takes RTMDet model outputs and converts them to bounding boxes.

    The bounding box locations are given as
    ...
    def __init__(self, score_threshold: float = 0.5, iou_threshold: float = 0.5) -> None:
        """Initialize the postprocessor.

        ## Args
        - score_threshold: the minimum confidence score that a detection should have. All predictions with a lower
                           score are classified as background.
    ...
    def process_single_batch_element(self, bbox_and_label_preds: BBoxLabelContainer) -> DetectionResult | None:
        ...
        return self._perform_nms(bboxes, classes, scores)

    @staticmethod
    def bbox_to_original_image(
        bboxes: torch.Tensor,
        preprocess_shape: torch.Size,
        orig_img_shape: torch.Size,
    ) -> torch.Tensor:
        """Transform bboxes in the preprocessed image shape back to the original image dimensions.
