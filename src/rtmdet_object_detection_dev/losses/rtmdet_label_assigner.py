...
class RTMDetLabelAssigner:
    """A class that turns ground truth labels to RTMDet target tensors so that RTMDet can be trained."""
    ...
    def assign_targets(self, ground_truths: list[BBoxLabelContainer], model_output: RTMDetOutput) -> BBoxLabelContainer:
        """Create target tensors to the ground truths based on the model predictions with simOTA label assignment."""
        ...
        for element_gts, pred_bboxes, pred_labels in zip(ground_truths, preds.bboxes, preds.labels, strict=True):
    ...
    def assign_targets_for_batch_element(
        self,
        element_gts: BBoxLabelContainer,
        element_preds: BBoxLabelContainer,
    ...
    def simOTA(  # noqa: N802
        self,
        ground_truths: BBoxLabelContainer,
        predictions: BBoxLabelContainer,
        grid_points: torch.Tensor,
    ) -> torch.Tensor:
        """Perform simOTA for the ground_truths and predictions.
        ...
        - pred_labels: tensor of shape (n, c). Predictions should be in the probability form,
                       i.e., each element in [0, 1]
        ...
        IoU_scores = torchvision.ops.box_iou(ground_truths.bboxes, predictions.bboxes)  # Shape (m, n)  # noqa: N806

        for i, (gt_bbox, gt_label, IoU_score) in enumerate(  # noqa: N806
            zip(ground_truths.bboxes, ground_truths.labels, IoU_scores, strict=True),
        ):
            sorted_IoUs, _ = IoU_score.sort(descending=True)  # noqa: N806
            top_k = max(
                1,
                int(sorted_IoUs[: self.q].sum().round()),
            )  # Note that q <= n. In all practical applications this should hold.
    ...
    def _make_assigner_matrix_feasible(self, assigner_matrix: torch.Tensor, cost_matrix: torch.Tensor) -> None:
        """Make sure that the assigner matrix is feasible.

        I.e., each column sum is at most one. Note that this implementation
        guarantees that each grid point predicts at most one ground truth but it is possible that some ground truth gets
        no grid points assigned to it. However, in practice this should be extremely rare.
        ...
        assigner_matrix[:, column_mask] = (
            0  # Assign one to the column element with lowest cost. All other elements to zero.
        )
