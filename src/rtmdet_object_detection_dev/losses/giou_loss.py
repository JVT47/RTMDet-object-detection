...
class GIoULoss(nn.Module):
    ...
    def forward(self, boxes_1: torch.Tensor, boxes_2: torch.Tensor) -> torch.Tensor:
        ...
        enclosure_width = torch.maximum(
            boxes_1[..., -2], boxes_2[..., -2]
        ) - torch.minimum(boxes_1[..., -4], boxes_2[..., -4])
        enclosure_height = torch.maximum(
            boxes_1[..., -1], boxes_2[..., -1]
        ) - torch.minimum(boxes_1[..., -3], boxes_2[..., -3])
        enclosure_area = enclosure_width * enclosure_height

        GIoU = IoU - (enclosure_area - union) / enclosure_area
        return 1 - GIoU

    def calc_IoU_and_union(
        self, boxes_1: torch.Tensor, boxes_2: torch.Tensor
        ...
        """
        I_width = torch.minimum(boxes_1[..., -2], boxes_2[..., -2]) - torch.maximum(
            boxes_1[..., -4], boxes_2[..., -4]
        )
        I_width = torch.maximum(torch.tensor([0]), I_width)
        I_height = torch.minimum(boxes_1[..., -1], boxes_2[..., -1]) - torch.maximum(
            boxes_1[..., -3], boxes_2[..., -3]
        )
        I_height = torch.maximum(torch.tensor([0]), I_height)
        I_area = I_width * I_height

        boxes_1_area = (boxes_1[..., -2] - boxes_1[..., -4]) * (
            boxes_1[..., -1] - boxes_1[..., -3]
        )
        boxes_1_area = torch.maximum(torch.tensor([0]), boxes_1_area)
        boxes_2_area = (boxes_2[..., -2] - boxes_2[..., -4]) * (
            boxes_2[..., -1] - boxes_2[..., -3]
        )
