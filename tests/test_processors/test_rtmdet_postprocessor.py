...
class TestRTMDetPostprocessor:
    ...
    def test_perform_nms(self) -> None:
        ...
        torch.testing.assert_close(
            detection_result.bboxes, torch.tensor([[0.0, 0, 3, 2], [0.0, 3, 1, 4]])
        )
    ...
    def test_process_single_batch_element(self) -> None:
        ...
        torch.testing.assert_close(
            detection_result.bboxes, torch.tensor([[0.0, 0, 2, 2], [0.0, 2, 1, 4]])
        )
    ...
    def test_process_batch(self) -> None:
        ...
        torch.testing.assert_close(
            detection_results[0].bboxes, torch.tensor([[7.0, 7, 8, 8]])
        )
        torch.testing.assert_close(detection_results[0].classes, torch.tensor([1]))
        torch.testing.assert_close(
            detection_results[0].scores, torch.tensor([1.0]), atol=1e-3, rtol=0.0
        )

        torch.testing.assert_close(
            detection_results[1].bboxes, torch.tensor([[16.0, 16, 17, 17]])
        )
    ...
    def test_bbox_to_original_image(self) -> None:
        ...
        bboxes = self.postprocessor.bbox_to_original_image(
            bboxes, preprocess_shape, orig_img_shape
        )
